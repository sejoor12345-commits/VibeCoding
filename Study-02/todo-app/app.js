// 전역 변수를 만들지 않도록 앱 전체를 함수 하나로 감싼다. 동작은 이전과 동일하다.
(function () {
  "use strict";
  ...
  const MAX_TEXT_LENGTH = 100; // 할 일 한 줄의 최대 글자 수
  const CLASSIFY_DEBOUNCE_MS = 250; // 자동 분류를 호출하기 전 입력이 멈추길 기다리는 시간
  const UNDO_TOAST_DURATION_MS = 5000; // 삭제 실행 취소 토스트가 떠 있는 시간
  const DEFAULT_TOAST_DURATION_MS = 3000; // 실행 취소가 없는 일반 토스트가 떠 있는 시간
  const QUOTE_FADE_MS = 150; // 격언이 사라졌다 나타나는 데 걸리는 시간 (왕복 0.3초)
  const PERCENT_COUNT_DURATION_MS = 600; // 퍼센트 숫자가 올라가거나 내려가는 데 걸리는 시간
  const CONFETTI_PIECE_COUNT = 30; // 완료 축하 색종이 조각 개수
  const CONFETTI_DURATION_MS = 2000; // 색종이 효과가 화면에 남아 있는 시간 (CSS의 2s와 맞춘다)
  ...
  function updateTodo(id, changes) {
    ...
    if (typeof safeChanges.text === "string") {
      safeChanges.text = safeChanges.text.slice(0, MAX_TEXT_LENGTH);
    }
  ...
  // 지금 텍스트 입력 칸(할 일 입력, 검색, 수정 입력창)에 포커스가 있는지 확인한다
  // (n, /, ? 단축키가 글자로 그대로 입력되지 않게 막는 데 쓴다)
  function isTypingInTextField() {
    const active = document.activeElement;
    if (!active) return false;
    if (active.isContentEditable) return true;
    if (active.tagName === "TEXTAREA") return true;
    return active.tagName === "INPUT" && active.type === "text";
  }

  // ===== classifier =====
  // 카테고리별 키워드 목록. 자주 쓰는 말이 있으면 이 배열에 바로 추가하면 된다.
  const KEYWORDS = {
    work: [
      "회의", "미팅", "보고서", "보고", "발표", "기획", "메일", "이메일", "견적", "계약",
      "제출", "마감", "출근", "고객", "거래처", "프로젝트", "검수", "결재", "회신", "업무",
      "팀장", "인수인계",
    ],
    personal: [
      "장보기", "병원", "약속", "운동", "헬스", "청소", "빨래", "요리", "은행", "택배",
      "생일", "선물", "여행", "예약", "가족", "친구", "미용실", "산책", "저녁", "데이트",
    ],
    study: [
      "공부", "복습", "예습", "시험", "문제", "강의", "인강", "과제", "암기", "단원",
      "독서", "논문", "자격증", "영어", "단어", "기출", "필기", "노트", "리포트", "학습",
    ],
  };
  ...
  function classifyTodo(text) {
    ...
    return topCategories[0].category;
  }
  ...
  // "다른 격언 보기"를 누르면 다음 격언으로 페이드 전환한다 (이 선택은 저장하지 않는다)
  ...
  // 이전 값에서 새 값까지 숫자를 부드럽게 세면서 콜백으로 전달한다
  ...
  function updateProgress() {
    ...
    animatePercentNumber(previousPercent, percent, PERCENT_COUNT_DURATION_MS, (value) => {
  ...
  // 진행률이 100%가 되는 순간 진행률 카드 위에 색종이 조각을 뿌리고 지운다
  function triggerConfetti() {
    ...
    window.setTimeout(() => container.remove(), CONFETTI_DURATION_MS);
  }
  ...
  function createTodoItem(todo) {
    ...
    handle.setAttribute("aria-label", "순서 바꾸기 (Alt와 위아래 화살표)");
    handle.tabIndex = 0;
    if (!reorderAllowed) {
  ...
  // 수정 중인 할 일을 입력창 + 카테고리 select로 보여준다 (자동 분류는 적용하지 않는다)
  ...
  function showToast({ message, actionLabel, onAction, duration = DEFAULT_TOAST_DURATION_MS }) {
  ...
  function isToastVisible() {
    return toastEl != null && toastEl.classList.contains("visible");
  }
  ...
  // 메뉴를 닫는다. 포커스가 메뉴 안에 있었을 때만(Esc나 항목 클릭) 더보기 버튼으로 되돌린다.
  ...
  function isMoreMenuOpen() {
    return !document.getElementById("more-menu").hidden;
  }

  // ----- 가져오기 확인 dialog -----
  let pendingImport = null; // 검증을 통과해 dialog에서 확인을 기다리는 가져오기 목록
  ...
  function handleFormSubmit(event) {
    ...
    // 할 일을 추가했으면 다음 입력을 위해 다시 자동 분류 상태로 돌아간다
    if (willAddSomething) {
      userPickedCategory = false;
      hideAutoClassifyLabel();
    }
  }
  ...
  // 메뉴가 열려 있을 때 위/아래 화살표로 항목 사이를 이동한다 (Esc는 전역 단축키 핸들러가 처리한다)
  ...
  // "단축키 보기" 메뉴 클릭을 처리한다
  function handleMenuShortcutsClick() {
    closeMoreMenu();
    openShortcutsDialog();
  }
  ...
  // ----- 할 일 자동 분류 (입력창에 쓰는 동안) -----
  let userPickedCategory = false; // 사용자가 칩을 직접 골랐는지 (고르면 이번 입력이 끝날 때까지 자동 분류가 덮어쓰지 않는다)
  let classifyDebounceId = null;

  // 자동 분류 결과 표시를 보여준다
  function showAutoClassifyLabel(category) {
    const labelEl = document.getElementById("auto-classify-label");
    labelEl.textContent = `자동 분류: ${CATEGORY_LABELS[category]}`;
    labelEl.hidden = false;
  }

  // 자동 분류 결과 표시를 숨긴다
  function hideAutoClassifyLabel() {
    const labelEl = document.getElementById("auto-classify-label");
    labelEl.hidden = true;
    labelEl.textContent = "";
  }

  // 카테고리 칩이 바뀔 때 0.2초 강조 효과를 재생한다
  function flashCategoryChip(category) {
    const label = document.querySelector(`label[for="category-${category}"]`);
    if (!label) return;
    label.classList.remove("chip-flash");
    void label.offsetWidth; // 강제로 리플로우를 일으켜 애니메이션이 처음부터 다시 재생되게 한다
  ...
  function handleTodoInputChange(event) {
    if (classifyDebounceId) clearTimeout(classifyDebounceId);
    const text = event.target.value;
    classifyDebounceId = window.setTimeout(() => {
      classifyDebounceId = null;
      applyAutoClassify(text);
    }, CLASSIFY_DEBOUNCE_MS);
  }

  // 사용자가 카테고리 칩을 직접 클릭하면(라디오 change) 자동 분류가 이번 입력엔 끼어들지 않게 한다
  function handleCategoryChipChange() {
    userPickedCategory = true;
    hideAutoClassifyLabel();
  }

  // ----- 키보드 단축키 -----
  // 입력창/검색창에 글을 쓰고 있지 않을 때만 n, /, ?가 동작한다. Esc는 항상 동작한다.
  // Ctrl/Cmd/Alt와 함께 눌리면 브라우저 자체 단축키와 겹치지 않도록 무시한다.
  function handleGlobalKeydown(event) {
    if (event.key === "Escape") {
      ...
      // <dialog>(가져오기 확인/단축키 도움말)는 열려 있으면 브라우저가 알아서 Esc로 닫는다
      return;
    }
  ...
  function init() {
    ...
    document.getElementById("todo-input").addEventListener("input", handleTodoInputChange);
    document.querySelector(".category-chips").addEventListener("change", handleCategoryChipChange);
