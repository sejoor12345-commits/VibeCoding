...
`google.colab.output.serve_kernel_port_as_iframe(8501)` with `--server.enableCORS false
--server.enableXsrfProtection false` (without them the Colab proxy gets rejected → endless loading; the
`_as_window` variant didn't work for the user) — see `PRD_step1.md` §8; `openrouter_client.py` as the single place holding `MODEL` and request/error
