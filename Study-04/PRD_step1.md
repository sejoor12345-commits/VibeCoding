...
- 올린 사진은 미리보기로 보인다 (F2에서 줄인 사진. AI에게 보내는 사진과 같다).
- JPG, PNG, WEBP 이외의 파일(예: 아이폰 HEIC)은 "JPG나 PNG로 바꿔서 올려주세요" 안내를 띄운다.
  허용하지 않는 확장자는 Streamlit이 영어 문구("files are not allowed")만 띄우므로, 업로드 칸 아래에
  "아이폰 HEIC 사진은 사진 앱에서 JPG로 내보내서 올려주세요. 파일 이름의 확장자만 바꾸면 열리지 않아요."를 늘 보여준다.
- 확장자만 바꾼 파일, 중간이 잘린 파일, 너무 큰 사진(2억 화소 등)처럼 열 수 없는 사진은
  "사진을 열 수 없어요. JPG나 PNG로 바꿔서 올려주세요."를 띄우고 "재료 인식하기" 버튼을 막는다.
...
- 사진을 올리면 바로 **한 번만** 줄여서 `st.session_state`에 보관하고(올린 파일의 `file_id`로 구분),
  미리보기와 재료 인식에 같이 쓴다. 화면이 다시 그려질 때마다 큰 원본 사진을 다시 푸는 일을 없애기 위해서다.

### F3. 재료 인식 요청
- "재료 인식하기" 버튼을 누르면 AI에 요청한다.
- 요청하는 동안 "AI가 사진 속 재료를 찾고 있어요... (보통 30초~1분, 최대 2분 걸려요. 기다리는 동안 다른 칸을
  바꾸거나 버튼을 누르면 요청이 취소돼요)"와 **지난 시간(초)**을 보여준다 (`st.spinner(..., show_time=True)`).
  Streamlit은 화면을 건드리면 하던 실행을 멈추고 처음부터 다시 실행해서, 기다리던 AI 답이 버려지기 때문이다.
- 제한 시간은 "서버가 조용한 시간"이 아니라 **요청 전체 시간**으로 잰다. OpenRouter는 답을 준비하는 동안
  몇 초마다 빈 글자를 보내 연결을 유지하므로, 조용한 시간만 재면 끝없이 기다리게 된다.
...
| 요청 후 **전체 시간**이 120초를 넘음 (`TIMEOUT_SECONDS`) | "응답이 120초 넘게 없어서 멈췄어요. 잠시 후 다시 시도해주세요." |
| OpenRouter에 연결하지 못함 (10초 안에 연결이 안 된 경우 포함) | "OpenRouter에 연결하지 못했어요. 인터넷 연결을 확인해주세요." |
| 열 수 없는 사진 | "사진을 열 수 없어요. JPG나 PNG로 바꿔서 올려주세요." (인식 버튼을 막음) |
...
- `openrouter_client.py`의 모델 이름은 상수(`VISION_MODEL`: 사진, `TEXT_MODEL`: 글자)로 두어,
