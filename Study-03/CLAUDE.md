...
- **`questions.js`** — the question bank. A single global array `questions`, loaded via a plain
  `<script>` tag (no modules, no bundler). 162 questions: 4 categories (한국사, 과학, 일반상식, 의학) x 4
  difficulties (하=초등학생, 중=고등학생, 상=대학생, 최상=그 분야를 공부하는 사람) x 10 questions each (한국사-하
  and 의학-최상 have 11 each). Each
